...
while k<=k_end 
    ...
    if u_mf<=u_mmid
        ...
        Delta_theta = del_psi(end)/E_psi(end);
        Delta_theta1 = del_psi1(end)/E_psi1(end);
        Delta_theta2 = del_psi2(end)/E_psi2(end);
        Delta_b = theta(end,12);
        Delta_ph = theta(end,5);
        g_theta1 = f1*1.5*F(end)*theta(end,2)*theta(end,7)/u_m(end)/E_psi1(end);
        g_theta2 = f2*1.5*F(end)*theta(end,4)*theta(end,9)/u_m(end)/E_psi2(end);
        g_theta = (E_psi1(end)*g_theta1+E_psi2(end)*g_theta2)/E_psi(end);
        gb = theta(end,13);
        gr = theta(end,6);
        %%%covariant energy density
    elseif u_mf<=u_mtr
        ...
        g_theta1 = 1.5*sqrt(F_a(end)).*theta_a(end,4)./u_m_a(end);
        g_theta2 = 1.5*sqrt(F_a(end)).*theta_a(end,6)./u_m_a(end)/mu;
        g_theta = (f1*epsilon01*g_theta1+f2*epsilon02*g_theta2)/...
            (f1*epsilon01+f2*epsilon02);
        D_r = theta_a(end,2)-theta_a(end,1)/2/u_m_a(end);
        gr = D_r/...
            (2*A_k/3/F_a(end)+(3+1/F_a(end))/4/u_m_a(end));
        gb = theta_a(end,9)/(2*A_k/3/F_a(end)+(3+1/F_a(end))/4/u_m_a(end));
        %%%parameters for save
    else
        %%%theta ode exact
        opts = odeset('MaxStep',res/mu);
        ...
        g_theta1 = 1.5*sqrt(F_3(end)).*theta_3(end,4)./u_m_3(end);
        g_theta2 = 1.5*sqrt(F_3(end)).*theta_3(end,6)./u_m_3(end)/mu;
        g_theta = (f1*epsilon01*g_theta1+f2*epsilon02*g_theta2)/...
            (f1*epsilon01+f2*epsilon02);
        gb = 0.75*(theta_3(end,2)+theta_3(end,9));
        gr = theta_3(end,2);
        %%%parameters for save
    ...
    Delta_theta1n(n) = Delta_theta1(end);
    Delta_theta2n(n) = Delta_theta2(end);
...
%%%save file
m1 = num2str(mr*1e-22);
m2 = num2str(mu*str2double(m1));
af = num2str(sqrt(u_mf/u_meq));
ki = num2str(k_initial);
kf = num2str(k_end);
angle1 = num2str(180 - angle_shift1);
angle2 = num2str(180 - angle_shift2);
kappa21str = num2str(kappa21);

filename = ['Axion_two_m1=' m1 '_m2=' m2 ...
    '_angle1=' angle1 '_angle2=' angle2 '_1to' ...
